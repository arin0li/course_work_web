...
document.addEventListener("keydown", (e) => {
 ...
 // Создаём карту
// Создаём карту
const map = L.map('dubossaryMap').setView([47.279, 29.127], 14);
...
const points = [
  {
    name: "Парк Энергетиков",
    coords: [47.27956758247646, 29.130527157513015],
    description: "Парк Энергетиков — тенистое, ухоженное пространство с ровными аллеями, детскими площадками и зелёными лужайками.",
    icon: parkIcon
  },
  {
    name: "Пляжная зона",
    coords: [47.27994854571172, 29.126958426263137],
    description: "Пляжная зона у водохранилища — лучшее место для купания и отдыха у «Дубоссарского моря».",
    icon: defaultIcon
  },
  {
    name: "Смотровая аллея над ГЭС",
    coords: [47.27804892850022, 29.12369284484913],
    description: "Смотровая аллея над ГЭС — панорама на водохранилище, где закаты окрашивают воду в медный цвет.",
    icon: gesIcon
  }
];
...
L.Routing.control({
  ...
  show: false,
  lineOptions: {
    styles: [{ color: '#2D5016', opacity: 1.0, weight: 8 }]
  },
...
document.getElementById('calcSubmit').addEventListener('click', function() {
  ...
  // === ТОЧКИ НА КАРТЕ ===
  const points = {
    park:    { name: "Парк Энергетиков", coords: [47.27956758247646, 29.130527157513015] },
    beach:   { name: "Пляжная зона", coords: [47.27994854571172, 29.126958426263137] },
    ges:     { name: "Смотровая аллея над ГЭС", coords: [47.27804892850022, 29.12369284484913] }
  };
  ...
  const routes = {
    light:    { 
      name: "Лайт (только Парк Энергетиков)", 
      distance: 2.5, // Прогулка по парку примерно 2.5 км
      difficulty: 1.0, // Очень легкий
      waypoints: [points.park],
      center: [47.27956758247646, 29.130527157513015]
    },
    standard: { 
      name: "Стандарт (Парк Энергетиков → Пляжная зона → Смотровая аллея над ГЭС)", 
      distance: calculateTotalDistance([points.park, points.beach, points.ges]),
      difficulty: 1.0, // Очень легкий
      waypoints: [points.park, points.beach, points.ges],
      center: [47.279, 29.127]
    }
  };
  ...
  document.getElementById('resultContent').innerHTML = `
    ...
    <p style="font-size:15px;line-height:1.6;color:#555;">
      ${r.difficulty === 1.0 && r.waypoints.length === 1 ? 'Прогулка по Парку Энергетиков — для тех, кто хочет насладиться тенистыми аллеями, зелёными лужайками и спокойной атмосферой парка.' :
        'Маршрут от Парка Энергетиков до смотровой аллеи над ГЭС — для тех, кто хочет провести полдня у воды, увидеть сосновую рощу, пляжную зону и насладиться панорамой «Дубоссарского моря».'}
    </p>
  `;
  ...
  setTimeout(() => {
    ...
    currentRoutingControl = L.Routing.control({
      ...
      lineOptions: { styles: [{ color: '#2D5016', weight: 8, opacity: 1.0 }] }
    }).addTo(resultMap);
