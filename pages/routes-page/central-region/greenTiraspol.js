...
document.addEventListener("keydown", (e) => {
 ...
 // Создаём карту
// Создаём карту
const map = L.map('greenTiraspolMap').setView([46.834, 29.626], 13);
...
const points = [
  {
    ...
    coords: [46.837902584563594, 29.61074609999923],
    description: "Екатерининский парк — ухоженные аллеи и зелёные зоны в центре Тирасполя.",
    icon: parkIcon
  },
  {
    name: "Ботанический сад",
    coords: [46.83087067392042, 29.64244202367993],
    description: "Ботанический сад — уникальная коллекция растений и тенистые дорожки.",
    icon: botanicIcon
  }
];
...
document.getElementById('calcSubmit').addEventListener('click', function() {
  ...
  // === ТОЧКИ НА КАРТЕ ===
  const points = {
    ekaterininsky: { name: "Екатерининский парк", coords: [46.837902584563594, 29.61074609999923] },
    botanic:       { name: "Ботанический сад",   coords: [46.83087067392042, 29.64244202367993] }
  };

  // Функция расчета расстояния между двумя точками (приблизительно)
  ...
  const routes = {
    light:    { 
      name: "Лайт (только Екатерининский)", 
      distance: 2.5, // Прогулка по парку примерно 2.5 км
      difficulty: 1.0, // Легкий
      waypoints: [points.ekaterininsky],
      center: [46.837902584563594, 29.61074609999923]
    },
    standard: { 
      name: "Стандарт (Екатерининский → Ботанический сад)", 
      distance: Math.round(calculateDistance(
        points.ekaterininsky.coords[0], 
        points.ekaterininsky.coords[1],
        points.botanic.coords[0], 
        points.botanic.coords[1]
      ) * 10) / 10, // Округляем до 1 знака после запятой
      difficulty: 1.3, // Средний
      waypoints: [points.ekaterininsky, points.botanic],
      center: [46.834, 29.626]
    }
  };
  ...
  document.getElementById('resultContent').innerHTML = `
    ...
    <p style="font-size:15px;line-height:1.6;color:#555;">
      ${r.difficulty === 1.0 ? 'Прогулка по Екатерининскому парку — для тех, кто ищет спокойствие в центре города и хочет насладиться вековыми деревьями и ухоженными аллеями.' :
        'Маршрут от Екатерининского парка до Ботанического сада — для любителей зелёных оазисов, где городская суета сменяется ароматом цветущих растений и пением птиц.'}
    </p>
  `;
